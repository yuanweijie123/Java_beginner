...
public class ArrayHeap<T> {
    ...
    private void bubbleDown(int index) {
        while (index < contents.size()
                && index != min(index, min(getLeftOf(index), getRightOf(index)))) {
    ...
    public T removeMin() {
        if (contents.size() < 2) {
            ...
            Node maxNode = removeNode(contents.size() - 1);
            setNode(1, maxNode);
            bubbleDown(1);
            return minNode.item();
        }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < size(); i++) {
            ...
            if (curNode.item().equals(item)) {
                double oldPriority = curNode.priority();
                curNode.setPriority(priority);
