...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue, rate, targetYear) * Math.pow((1 - inflationRate * 0.01), numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double saving = 0;
        for (int year = 0; year <= numYears(targetYear); year += 1) {
            saving += perYear * Math.pow((1 + rate * 0.01), year);
        }
        return saving;
    }
