...
class BoardWidget extends Pad {
    ...
    static final Color
        ...
        TRANS_SQUARE_COLOR = new Color(238, 207, 161, 0),
    ...
    private void drawGrid(Graphics2D g) {
        ...
        if (clickSq != null) {
            ...
            g.fillRect(cx(clickSq), cy(clickSq), SQUARE_SIDE, SQUARE_SIDE);
        }
        ...
        for (int i = 0; i < SIZE; i++) {
            ...
            g.drawString(Character.toString((char) (X_FONT - i)),
                    8, SQUARE_SIDE / 2 + SQUARE_SIDE * i + 7);
        }

        for (int i = 0; i < SIZE; i++) {
            g.setColor(Color.black);
            g.setFont(ROW_COL_FONT);
            g.drawString(Character.toString((char) (Y_FONT + i)),
                    SQUARE_SIDE / 2 + SQUARE_SIDE * i + MARGIN,
                    BOARD_SIDE - 5);
        }
    }
    ...
    private void drawPiece(Graphics2D g, Square s) {
        Color c;
        if (_board.get(s.col(), s.row()).side() == BLACK) {
            c = BLACK_COLOR;
        } else if (_board.get(s.col(), s.row()).side() == WHITE) {
            c = WHITE_COLOR;
        } else {
            c = TRANS_SQUARE_COLOR;
        }
        int px = cx(s.row()), py = cy(s.col());
        if (c == BLACK_COLOR || c == WHITE_COLOR) {
            g.setColor(Color.red);
            g.drawOval(px + MARGIN_OVAL, py + MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL);
        }
        g.setColor(c);
        if (_board.get(s.col(), s.row()).equals(KING)) {
            g.fillOval(px + MARGIN_OVAL, py + MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL);
            g.setColor(Color.red);
            g.setFont(KING_FONT);
            g.drawString("K", px + KING_X, py + KING_Y);
        } else {
            g.fillOval(px + MARGIN_OVAL, py + MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL,
                    SQUARE_SIDE - 2 * MARGIN_OVAL);
    ...
    private void click(Square s) {
        if (clickSq != null) {
            if (_board.isLegal(sq(clickSq.row(), clickSq.col()),
                    sq(s.row(), s.col()))) {
                _board.makeMove(sq(clickSq.row(), clickSq.col()),
                        sq(s.row(), s.col()));
            }
            clickSq.clickMe();
            clickSq = null;
