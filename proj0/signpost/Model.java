...
import static signpost.Place.dirOf;
...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < last; i++) {
            ...
            if (i + 1 == last) {
                ...
                dir = p.dirOf(_solnNumToPlace[i + 1]);
            }
            boolean fix = (i == 0 || i == last - 1);
            if (i == 0) {
                sequenceNum = 1;
            } else if (i == last - 1) {
                sequenceNum = last;
            ...
            Sq sq = new Sq(p.x, p.y, sequenceNum, fix, dir, -1);
            _board[p.x][p.y] = sq;
        ...
        for (int i = 0; i < last; i++) {
            if (_solnNumToPlace[i] == null) {
                throw badArgs("Sequence number " + (i + 1) + "missing.");
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq sq = _board[i][j];
                sq._successors = allSuccessors(i, j, sq._dir);
                for (Place pl : sq._successors) {
                    Sq sqSub = get(pl);
                    sqSub._predecessors.add(pl(i, j));
                }
            }
        }
    ...
    Model(Model model) {
        ...
        _board = Arrays.stream(model._board)
                .map(Sq[]::clone).toArray(Sq[][]::new);
        ...
        for (Place p : _solnNumToPlace) {
            Sq sqNew = get(p);
            Sq sq = model.get(p);
            sqNew._head = get(sq._head.pl);
            if (sq._successor != null) {
                sqNew._successor = get(sq._successor.pl);
            }
            if (sq._predecessor != null) {
                sqNew._predecessor = get(sq._predecessor.pl);
            }
        }
    }
            ...
            for (int j = 0; j < _height; j++) {
                ...
                if (sq._successor == null && sq._sequenceNum != 0) {
                    for (Sq nextSq : _allSquares) {
                        if (nextSq._sequenceNum == sq._sequenceNum + 1) {
                            sq.connect(nextSq);
                            flag = true;
                        }
                    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        Sq cur = get(x, y);
        if (cur._successor != null) {
            return dirOf(cur.x, cur.y, cur._successor.x, cur._successor.y);
        }
        return 0;
    }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0) {
                Sq itr = this;
                while (itr.successor() != null) {
                    itr.successor()._sequenceNum = itr.sequenceNum() + 1;
                    itr = itr.successor();
                }
            }
            ...
            if (s1._sequenceNum != 0) {
                Sq itr = s1;
                while (itr.predecessor() != null && itr.sequenceNum() > 0) {
                    itr.predecessor()._sequenceNum = itr.sequenceNum() - 1;
                    itr = itr.predecessor();
                }
            }
            ...
            Sq itr = this;
            while (itr.successor() != null) {
                itr.successor()._head = this.head();
                itr = itr.successor();
            }

            if (_sequenceNum == 0 && s1._sequenceNum == 0) {
                this._head._group = joinGroups(_head._group, sgroup);
            }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                disconnectBackward();
                disconnectForward(next);
            }
            Sq nextItr = next;
            while (nextItr != null) {
                nextItr._head = next;
                nextItr = nextItr._successor;
            }
        }

        /**
         * Utils for connect in backward direction.
         */
        public void disconnectBackward() {
            boolean preFixNum = this.hasFixedNum();
            Sq preItr = this;
            while (preItr.predecessor() != null) {
                preFixNum = preFixNum || preItr.predecessor().hasFixedNum();
                preItr = preItr.predecessor();
            }
            if (!preFixNum) {
                preItr = this;
                this._sequenceNum = 0;
                if (this.predecessor() == null) {
                    ...
                    while (preItr.predecessor() != null) {
                        preItr.predecessor()._sequenceNum = 0;
                        preItr._group = newGroup;
                        preItr = preItr.predecessor();
                    }
                }
            }
        ...
        /**
         * @param next
         * Utils for disconnect in forward direction
         */
        public void disconnectForward(Sq next) {
            boolean nextFixNum = next.hasFixedNum();
            Sq nextItr = next;
            while (nextItr.successor() != null) {
                nextFixNum = nextFixNum
                        || nextItr.successor().hasFixedNum();
                nextItr = nextItr.successor();
            }
            if (!nextFixNum) {
                nextItr = next;
                next._sequenceNum = 0;
                if (next.successor() == null) {
                    ...
                    int newGroup = newGroup();
                    while (nextItr.successor() != null) {
                        nextItr.successor()._sequenceNum = 0;
                        nextItr._group = newGroup;
                        nextItr = nextItr.successor();
                    }
                }
            }
