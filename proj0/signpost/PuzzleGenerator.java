        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place p : sq.successors()) {
                    Sq sqCandidate = model.get(p);
                    if (sq.connectable(sqCandidate)) {
                        nFound += 1;
                        found = sqCandidate;
                        if (sq.hasFixedNum() && sqCandidate.hasFixedNum()) {
                            nFound = 1;
                            found = sqCandidate;
                            break;
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place plCandidate : sq.predecessors()) {
                    Sq sqCandidate = model.get(plCandidate);
                    if (sqCandidate.connectable(sq)) {
                        nFound += 1;
                        found = sqCandidate;
                        if (sq.hasFixedNum() && sqCandidate.hasFixedNum()) {
                            nFound = 1;
