 ...
 *
 *  @author Josh Hug and Weijie Yuan
 */
...
public class MatrixUtils {
        ...
        for (int i = 1; i< row_num; i ++) {
            for (int j = 0; j < col_num; j++) {
                ...
                for (int k : new int[]{-1, 0, 1}) {
                    int candidate_col = j + k;
                    if (candidate_col >= 0 && candidate_col < col_num){
                        min = Math.min(min, c[i-1][candidate_col]);
                    }
                }
                c[i][j] += min;
            }
    ...
    public static double[][] transpose(double[][] m){
        int row_num = m.length;
        int col_num = m[0].length;
        double[][] mT = new double[col_num][row_num];
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            double[][] res = accumulateVertical(mT);
     ...
     */
    public static int findIndexMin(double[] l, int lo, int hi) {
        double min = l[lo];
        int indexMin = lo;
    ...
    public static int[] findVerticalSeam(double[][] m) {
        int row_num = m.length;
        int col_num = m[0].length;
        int[] seam = new int[row_num];
        seam[row_num - 1] = findIndexMin(m[row_num - 1],0,col_num-1);
        for (int i = row_num - 2; i >= 0; i--){
            seam[i] = findIndexMin(m[i],
                    Math.max(seam[i + 1] - 1, 0),
                    Math.min(seam[i + 1] + 1, col_num - 1)
            );
        }
        return seam; //your code here
    }
